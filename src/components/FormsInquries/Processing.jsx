...
import { billStatus, companyNames, orderType, billName } from "../../constants/data";
...
export default function Processing({ fetchData, data, setReject, changeStatus, close }) {
    ...
    const handleAccsept = async (e) => {
        ...
        await axios.request(
            {
                url: `${baseUrl}${"send-inquiry-result"}/${data.id}`,
                method: "post",
                data: {
                    admin_note: notes,
                    amount: amount
        ...
        {data?.billable?.governorate && <li className="p-2 border-b border-[#A6A6A6] text-sm">
          المحافظة  : <span>{data?.billable?.governorate}</span>
            ...
            <div className="w-full text-right mt-4">
                <label htmlFor="amount" className="text-right mb-2 text-xs">: الكمية</label>
                <input
                    id="amount"
